...
ImageNet processing script like this one is useful
...
Latent feature extraction example:
```bash
CUDA_VISIBLE_DEVICES=4,6 python scripts/extract_features_jax.py --data-path /media/data4/ImageNet/train/ --batch-size 64 --image-size 512 
```

Training example
```bash
bash scripts/launch_tpu_guandao.sh
```

Inference example:
```bash
CUDA_VISIBLE_DEVICES=0,1 python sample_ddp_jax.py \
    --resume results/DiT-Jax-flaxfeat-DiT-S-2-mttt-2024-Jun-10-16-29-38/ \
