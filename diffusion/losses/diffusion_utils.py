...
def normal_kl(mean1, logvar1, mean2, logvar2):
    ...
    """
    # NOTE: this check is at Torch, discarded
    # tensor = None
...
def discretized_gaussian_log_likelihood(x, *, means, log_scales, eps=1e-8):
    """
    ...
    log_cdf_plus = jnp.log(jnp.clip(cdf_plus, min=eps))
    log_one_minus_cdf_min = jnp.log(jnp.clip(1.0 - cdf_min, min=eps))
    ...
    log_probs = jnp.where(
        ...
        jnp.where(
            ...
            jnp.log(jnp.clip(cdf_delta, min=eps))),
    )
