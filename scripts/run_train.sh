...
export CODE_DIR=/mnt/disks/nfs/ujinsong/jax-diffusion

# directory that stores the checkpoints upon training
export OUT_DIR=/mnt/disks/gs/sci-guandao
...
XLA_PYTHON_CLIENT_MEM_FRACTION=.95 PYTHONPATH=${CODE_DIR} WANDB_DIR=${WANDB_DIR} python ${CODE_DIR}/train.py \
  --config-name config \
  model=dit_l \
  expr_name="release/" \
  data_dir=${DATA_DIR} \
